...
def _apply_relational_fallback(vector_config, relational_config):
    if vector_config.get("vector_db_provider") != "pgvector":
        return vector_config

    merged = dict(vector_config)
    port = merged.get("vector_db_port")
    merged["vector_db_host"] = merged.get("vector_db_host") or relational_config.db_host
    merged["vector_db_port"] = (
        relational_config.db_port if port in (None, "", 1234, "1234") else port
    )
    merged["vector_db_username"] = (
        merged.get("vector_db_username") or relational_config.db_username
    )
    merged["vector_db_password"] = (
        merged.get("vector_db_password") or relational_config.db_password
    )
...
async def create_pg_database(vector_config):
    ...
    vector_config = _apply_relational_fallback(vector_config, get_relational_config())
...
async def delete_pg_database(dataset_database: DatasetDatabase):
    ...
    vector_config = _apply_relational_fallback(vector_config.to_dict(), get_relational_config())
    # Create a maintenance engine, used when creating new postgres databases.
    # Database named "postgres" should always exist. We need this since the SQLAlchemy
